  ...
  http.createServer(function (req, resp) {
   ...
   console.log(`    -> Sending request to http://localhost:8000${req.url}...`)
    fetch('http://localhost:8000' + req.url).then(function (incomingResp) {
          ...
          if(incomingResp.statusCode = 402) {
            ...
            const listenerForThisTransfer = function (transfer, fulfillment) {
              if (transfer.id === transferId) {
                ...
                //Get real request
                console.log(`    -> Sending request to http://localhost:8000/${fulfillment}...`)
            ...
            console.log(`    - Making payment to ${destinationAddress} with condition ${condition}`)
            plugin.sendTransfer({
              ...
              console.log(`    - Transfer (tx-id:${transferId}) prepared, waiting for fulfillment...`)
            }, function (err) {
        ...
        console.log(` 3. Visit http://localhost:8001 in your browser to buy a letter`)
      })
})